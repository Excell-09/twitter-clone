...
const SidebarLinks = ({ Icon, title, access = false, onClick }: Props) => {
  return (
    <div
      onClick={() => onClick?.()}
      className={`hidden md:flex items-center space-x-2 px-4 py-3 rounded-full hover:bg-gray-100 transition-all duration-100 group max-w-fit ${access ? 'cursor-pointer' : 'cursor-not-allowed'}`}>
